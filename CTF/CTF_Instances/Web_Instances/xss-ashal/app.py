...
AREAS = [
    {
        ...
        "name": "Basic Alert",
        "flag": FLAGS[1],
        "check": lambda msg: "alert(" in msg,  # detect alert
    ...
    },
    {
        ...
        "flag": FLAGS[2],
        "check": lambda msg: "document.cookie" in msg,
        "payload_hint": "<script>alert(document.cookie)</script>"
    },
    {
        "id": 3,
        "name": "Session Hijacking",
        "flag": FLAGS[3],
        "check": lambda msg: "fetch" in msg or "XMLHttpRequest" in msg,
        "payload_hint": "<script>fetch('/update_status',{method:'POST',body:'status=hacked'})</script>"
    },
    {
        "id": 4,
        "name": "Keylogging",
        "flag": FLAGS[4],
        "check": lambda msg: "onkeypress" in msg,
        "payload_hint": "<script>document.onkeypress=function(e){alert(e.key)}</script>"
    },
    {
        "id": 5,
        "name": "Targeted Defacement",
        "flag": FLAGS[5],
        "check": lambda msg: "getElementById" in msg and "innerHTML" in msg,
        "payload_hint": "<script>document.getElementById('inscriptions-container').innerHTML = '<h1>Defaced!</h1>'</script>"
    }
]

SERIALIZABLE_AREAS = [
    {"id": a["id"], "name": a["name"], "flag": a["flag"]} for a in AREAS
]
...
def index():
    ...
    solved_rows = db.execute(
        "SELECT solved_area_id FROM user_progress WHERE user_id = ?",
        (user_id,)
    ).fetchall()
    ...
    if request.method == 'POST':
        ...
        if name and message:
            db.execute(
                "INSERT INTO inscriptions (user_id, name, message, timestamp) VALUES (?, ?, ?, ?)",
                (user_id, name, message, time.strftime('%Y-%m-%d %H:%M:%S'))
            )
            db.commit()
            ...
            for area in AREAS:
                if area['id'] not in solved and area['check'](message):
                    db.execute(
                        "INSERT INTO user_progress (user_id, solved_area_id) VALUES (?, ?)",
                        (user_id, area['id'])
                    )
                    db.commit()
    ...
    inscriptions = db.execute(
        "SELECT name, message, timestamp FROM inscriptions WHERE user_id = ? ORDER BY id DESC",
        (user_id,)
    ).fetchall()
