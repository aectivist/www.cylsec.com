...
def add_challenge():
    ...
    # Update instance choices based on challenge type
    form.instance_name.choices = [('', '-- No Instance --')]
    if form.type.data:
        from app.instance_manager import get_available_instances
        instances = get_available_instances(form.type.data)
        for instance in instances:
            form.instance_name.choices.append((instance, instance))
...
def edit_challenge(challenge_id):
    ...
    # Update instance choices based on challenge type
    form.instance_name.choices = [('', '-- No Instance --')]
    challenge_type = form.type.data or chal.type
    if challenge_type:
        from app.instance_manager import get_available_instances
...
@login_required
@maker_required
def api_get_instances(challenge_type):
    """API endpoint to get available instances for a challenge type."""
    from app.instance_manager import get_available_instances
