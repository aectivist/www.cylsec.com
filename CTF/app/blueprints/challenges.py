...
from app.models import Category, Challenge, Solve, DockerInstance
...
def view_challenge(challenge_id):
    ...
    # Resolve active Docker instance for this user+challenge so templates
    # don't have to call Python methods inside Jinja selectattr filters.
    active_instance = None
    if chal.instance_name:
        active_instance = DockerInstance.query.filter(
            DockerInstance.user_id == current_user.id,
            DockerInstance.challenge_id == challenge_id,
            DockerInstance.is_active == True,
            DockerInstance.expires_at > datetime.utcnow(),
        ).first()

    return render_template('challenge_detail.html', chal=chal, active_instance=active_instance)
