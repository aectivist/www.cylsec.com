...
def send_registration_confirmation_email(email, token, username):
    ...
    msg = Message(subject, sender=current_app.config.get('MAIL_DEFAULT_SENDER') or current_app.config.get('MAIL_USERNAME'), recipients=[email])
    msg.html = html
    try:
        mail.send(msg)
    except Exception as e:
        current_app.logger.error(f"Failed to send registration confirmation email to {email}: {str(e)}")
        # Re-raise so caller can handle it
        raise
...
def send_otp_email(user, purpose, otp):
    ...
    msg = Message(subject, sender=current_app.config.get('MAIL_DEFAULT_SENDER') or current_app.config.get('MAIL_USERNAME'), recipients=[user.email])
    msg.html = html
    try:
        mail.send(msg)
    except Exception as e:
