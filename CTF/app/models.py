...
class Challenge(db.Model):
    ...
    # Instance-based challenges (references CTF_Instances folder)
    instance_name = db.Column(db.String(255), nullable=True)  # Folder name in CTF_Instances/{type}_Instances/
...
class DockerInstance(db.Model):
    """Tracks temporary Docker instances spawned for CTF challenges."""
    __tablename__ = 'docker_instances'
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False)
    challenge_id = db.Column(db.Integer, db.ForeignKey('challenges.id'), nullable=False)
    container_id = db.Column(db.String(255), unique=True, nullable=False)  # Docker container ID
    port = db.Column(db.Integer, nullable=False)  # Assigned external port
    expires_at = db.Column(db.DateTime, nullable=False)  # Time limit expiration
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    last_extended_at = db.Column(db.DateTime, nullable=True)
    extensions_used = db.Column(db.Integer, default=0)  # Track extension count
    is_active = db.Column(db.Boolean, default=True)
    url = db.Column(db.String(255), nullable=True)  # Full URL for access
    ...
    def can_extend(self, max_extensions=3):
        """Check if user can extend this instance."""
